...
for foldername in ['Healthy','Schizophrenic']:
    ...
    for i in range(len(data_files)):
        ...
        for i in range(1,len(file['subject']),3072):
            ...
            os.makedirs(fr'{og_dir}\CNN_Data\{foldername}',exist_ok='true')
            os.chdir(fr'{og_dir}\CNN_data\{foldername}')
            plt.specgram(Fz,1024)
            plt.savefig(fr'{subj}_{trial}_{condition}_Fz.png')
            plt.specgram(FC3,1024)
            plt.savefig(fr'{subj}_{trial}_{condition}_FC3.png')
            plt.specgram(FC4,1024)
            plt.savefig(fr'{subj}_{trial}_{condition}_FC4.png')
            plt.specgram(C3,1024)
            plt.savefig(fr'{subj}_{trial}_{condition}_C3.png')
            plt.specgram(FCz,1024)
            plt.savefig(fr'{subj}_{trial}_{condition}_FCz.png')
            plt.specgram(Cz,1024)
            plt.savefig(fr'{subj}_{trial}_{condition}_Cz.png')
            plt.specgram(C4,1024)
            plt.savefig(fr'{subj}_{trial}_{condition}_C4.png')
            plt.specgram(CP3,1024)
            plt.savefig(fr'{subj}_{trial}_{condition}_CP3.png')
            plt.specgram(CP4,1024)
            plt.savefig(fr'{subj}_{trial}_{condition}_CP4.png')
